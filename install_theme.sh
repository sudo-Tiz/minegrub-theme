...
## Theme Installation
if [[ "$INTERACTIVE_MODE" = true && "$INSTALL_THEME" = false ]]; then
  read -p "Install Theme ? [y/N] " -en 1 install_theme
  [[ "$install_theme" =~ y|Y ]] && INSTALL_THEME=true
fi
